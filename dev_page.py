...
# Create a Blueprint for the dev page
...
@dev_bp.route('/dev_page')
def dev_page():
    # Check if the user has permission to view this page
    if 'dev' not in session['permissions'] and 'admin' not in session['permissions']:
    ...
    try:
        ...
        # Fetch records where the reference_name is 'dev', case-insensitive
        cursor.execute('SELECT * FROM customers WHERE LOWER(reference_name) = "dev" ORDER BY id DESC')
        ...
        return render_template('dev/dev.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return "An error occurred while fetching data."

@dev_bp.route('/dev/edit/<int:customer_id>', methods=['GET', 'POST'])
def dev_edit_page(customer_id):
    if 'dev' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render_template('dev/dev_edit.html', customer=customer_dict)
        ...
        if request.method == 'POST':
            ...
            if referrer:
                ...
                return redirect(url_for('dev_bp.dev_delivery_page'))  # Fallback to a default page (adjust as needed)
...
@dev_bp.route('/dev/delivery', methods=['GET'])
def dev_delivery_page():
    if 'dev' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if search_query:
            cursor.execute(''' 
                ...
                WHERE LOWER(reference_name) = "dev" 
        ...
        return render_template('dev/dev_delivery.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return f"An error occurred while fetching data: {e}"

@dev_bp.route('/dev/dashboard', methods=['GET'])
def dev_dashboard():
    if 'dev' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "dev" AND LOWER(delivery) = "yes"')
        total_customers = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "dev" AND LOWER(delivery) = "yes"')
        total_amount = cursor.fetchone()[0] or 0

        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "dev" AND LOWER(delivery) = "no"')
        total_no_delivery = cursor.fetchone()[0]

        # Fetch total amount for "no" deliveries (including canceled orders for totals)
        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "dev" AND LOWER(delivery) = "no"')
        total_no_delivery_amount = cursor.fetchone()[0] or 0

        # Fetch the same for canceled deliveries (to include in total)
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "dev" AND payment_status = "Cancel"')
        total_cancelled = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "dev" AND payment_status = "Cancel"')
        total_cancelled_amount = cursor.fetchone()[0] or 0

        conn.close()

        # Render the dashboard template with the statistics and recent changes
        return render_template('dev/dev_dashboard.html',
...
@dev_bp.route('/dev/payment_due', methods=['GET'])
def payment_due():
    if 'dev' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        # Fetch customers where reference_name = 'dev', delivery = 'yes', and payment_status = 'pending'
        cursor.execute('''
            SELECT * FROM customers
            WHERE LOWER(reference_name) = "dev"
            AND LOWER(delivery) = "yes"
        ...
        return render_template('dev/payment_due.html', 
...
@dev_bp.route('/dev/scheme', methods=['GET', 'POST'])
def dev_scheme_page():
    if 'dev' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
    ...
    try:
        ...
        # Query to count orders grouped by amount for dev, filtering by date range
        cursor.execute(''' 
...
@dev_bp.route('/dev/canceled', methods=['GET'])
def dev_canceled_page():
    if 'dev' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Query to get the canceled orders for dev, sorted by date (most recent first)
        cursor.execute('''
        ...
        return render_template('dev/dev_canceled_page.html', canceled_orders=canceled_orders)
