...
# Create a Blueprint for the pooja page
pooja_bp = Blueprint('pooja_bp', __name__)
...
@pooja_bp.route('/pooja_page')
def pooja_page():
    # Check if the user has permission to view this page
    if 'pooja' not in session['permissions'] and 'admin' not in session['permissions']:
    ...
    try:
        ...
        # Fetch records where the reference_name is 'pooja', case-insensitive
        cursor.execute('SELECT * FROM customers WHERE LOWER(reference_name) = "pooja" ORDER BY id DESC')
        ...
        return render_template('pooja/pooja.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return "An error occurred while fetching data."

@pooja_bp.route('/pooja/edit/<int:customer_id>', methods=['GET', 'POST'])
def pooja_edit_page(customer_id):
    if 'pooja' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render_template('pooja/pooja_edit.html', customer=customer_dict)
        ...
        if request.method == 'POST':
            ...
            if referrer:
                ...
                return redirect(url_for('pooja_bp.pooja_delivery_page'))  # Fallback to a default page (adjust as needed)
...
@pooja_bp.route('/pooja/delivery', methods=['GET'])
def pooja_delivery_page():
    if 'pooja' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if search_query:
            cursor.execute(''' 
                ...
                WHERE LOWER(reference_name) = "pooja" 
        ...
        return render_template('pooja/pooja_delivery.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return f"An error occurred while fetching data: {e}"

@pooja_bp.route('/pooja/dashboard', methods=['GET'])
def pooja_dashboard():
    if 'pooja' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "pooja" AND LOWER(delivery) = "yes"')
        total_customers = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "pooja" AND LOWER(delivery) = "yes"')
        total_amount = cursor.fetchone()[0] or 0

        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "pooja" AND LOWER(delivery) = "no"')
        total_no_delivery = cursor.fetchone()[0]

        # Fetch total amount for "no" deliveries (including canceled orders for totals)
        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "pooja" AND LOWER(delivery) = "no"')
        total_no_delivery_amount = cursor.fetchone()[0] or 0

        # Fetch the same for canceled deliveries (to include in total)
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "pooja" AND payment_status = "Cancel"')
        total_cancelled = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "pooja" AND payment_status = "Cancel"')
        total_cancelled_amount = cursor.fetchone()[0] or 0

        conn.close()

        # Render the dashboard template with the statistics and recent changes
        return render_template('pooja/pooja_dashboard.html',
...
@pooja_bp.route('/pooja/payment_due', methods=['GET'])
def payment_due():
    if 'pooja' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        # Fetch customers where reference_name = 'pooja', delivery = 'yes', and payment_status = 'pending'
        cursor.execute('''
            SELECT * FROM customers
            WHERE LOWER(reference_name) = "pooja"
            AND LOWER(delivery) = "yes"
        ...
        return render_template('pooja/payment_due.html', 
...
@pooja_bp.route('/pooja/scheme', methods=['GET', 'POST'])
def pooja_scheme_page():
    if 'pooja' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
    ...
    try:
        ...
        # Query to count orders grouped by amount for pooja, filtering by date range
        cursor.execute(''' 
...
@pooja_bp.route('/pooja/canceled', methods=['GET'])
def pooja_canceled_page():
    if 'pooja' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Query to get the canceled orders for pooja, sorted by date (most recent first)
        cursor.execute('''
        ...
        return render_template('pooja/pooja_canceled_page.html', canceled_orders=canceled_orders)
