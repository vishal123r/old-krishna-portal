...
# Create a Blueprint for the harish page
harish_bp = Blueprint('harish_bp', __name__)
...
@harish_bp.route('/harish_page')
def harish_page():
    # Check if the user has permission to view this page
    if 'harish' not in session['permissions'] and 'admin' not in session['permissions']:
    ...
    try:
        ...
        # Fetch records where the reference_name is 'harish', case-insensitive
        cursor.execute('SELECT * FROM customers WHERE LOWER(reference_name) = "harish" ORDER BY id DESC')
        ...
        return render_template('harish/harish.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return "An error occurred while fetching data."

@harish_bp.route('/harish/edit/<int:customer_id>', methods=['GET', 'POST'])
def harish_edit_page(customer_id):
    if 'harish' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render_template('harish/harish_edit.html', customer=customer_dict)
        ...
        if request.method == 'POST':
            ...
            if referrer:
                ...
                return redirect(url_for('harish_bp.harish_delivery_page'))  # Fallback to a default page (adjust as needed)
...
@harish_bp.route('/harish/delivery', methods=['GET'])
def harish_delivery_page():
    if 'harish' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if search_query:
            cursor.execute(''' 
                ...
                WHERE LOWER(reference_name) = "harish" 
        ...
        return render_template('harish/harish_delivery.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return f"An error occurred while fetching data: {e}"

@harish_bp.route('/harish/dashboard', methods=['GET'])
def harish_dashboard():
    if 'harish' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "harish" AND LOWER(delivery) = "yes"')
        total_customers = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "harish" AND LOWER(delivery) = "yes"')
        total_amount = cursor.fetchone()[0] or 0

        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "harish" AND LOWER(delivery) = "no"')
        total_no_delivery = cursor.fetchone()[0]

        # Fetch total amount for "no" deliveries (including canceled orders for totals)
        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "harish" AND LOWER(delivery) = "no"')
        total_no_delivery_amount = cursor.fetchone()[0] or 0

        # Fetch the same for canceled deliveries (to include in total)
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "harish" AND payment_status = "Cancel"')
        total_cancelled = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "harish" AND payment_status = "Cancel"')
        total_cancelled_amount = cursor.fetchone()[0] or 0

        conn.close()

        # Render the dashboard template with the statistics and recent changes
        return render_template('harish/harish_dashboard.html',
...
@harish_bp.route('/harish/payment_due', methods=['GET'])
def payment_due():
    if 'harish' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        # Fetch customers where reference_name = 'harish', delivery = 'yes', and payment_status = 'pending'
        cursor.execute('''
            SELECT * FROM customers
            WHERE LOWER(reference_name) = "harish"
            AND LOWER(delivery) = "yes"
        ...
        return render_template('harish/payment_due.html', 
...
@harish_bp.route('/harish/scheme', methods=['GET', 'POST'])
def harish_scheme_page():
    if 'harish' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
    ...
    try:
        ...
        # Query to count orders grouped by amount for harish, filtering by date range
        cursor.execute(''' 
...
@harish_bp.route('/harish/canceled', methods=['GET'])
def harish_canceled_page():
    if 'harish' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Query to get the canceled orders for harish, sorted by date (most recent first)
        cursor.execute('''
        ...
        return render_template('harish/harish_canceled_page.html', canceled_orders=canceled_orders)
