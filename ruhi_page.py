...
# Create a Blueprint for the ruhi page
ruhi_bp = Blueprint('ruhi_bp', __name__)
...
@ruhi_bp.route('/ruhi_page')
def ruhi_page():
    # Check if the user has permission to view this page
    if 'ruhi' not in session['permissions'] and 'admin' not in session['permissions']:
    ...
    try:
        ...
        # Fetch records where the reference_name is 'ruhi', case-insensitive
        cursor.execute('SELECT * FROM customers WHERE LOWER(reference_name) = "ruhi" ORDER BY id DESC')
        ...
        return render_template('ruhi/ruhi.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return "An error occurred while fetching data."

@ruhi_bp.route('/ruhi/edit/<int:customer_id>', methods=['GET', 'POST'])
def ruhi_edit_page(customer_id):
    if 'ruhi' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render_template('ruhi/ruhi_edit.html', customer=customer_dict)
        ...
        if request.method == 'POST':
            ...
            if referrer:
                ...
                return redirect(url_for('ruhi_bp.ruhi_delivery_page'))  # Fallback to a default page (adjust as needed)
...
@ruhi_bp.route('/ruhi/delivery', methods=['GET'])
def ruhi_delivery_page():
    if 'ruhi' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if search_query:
            cursor.execute(''' 
                ...
                WHERE LOWER(reference_name) = "ruhi" 
        ...
        return render_template('ruhi/ruhi_delivery.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return f"An error occurred while fetching data: {e}"

@ruhi_bp.route('/ruhi/dashboard', methods=['GET'])
def ruhi_dashboard():
    if 'ruhi' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "ruhi" AND LOWER(delivery) = "yes"')
        total_customers = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "ruhi" AND LOWER(delivery) = "yes"')
        total_amount = cursor.fetchone()[0] or 0

        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "ruhi" AND LOWER(delivery) = "no"')
        total_no_delivery = cursor.fetchone()[0]

        # Fetch total amount for "no" deliveries (including canceled orders for totals)
        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "ruhi" AND LOWER(delivery) = "no"')
        total_no_delivery_amount = cursor.fetchone()[0] or 0

        # Fetch the same for canceled deliveries (to include in total)
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "ruhi" AND payment_status = "Cancel"')
        total_cancelled = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "ruhi" AND payment_status = "Cancel"')
        total_cancelled_amount = cursor.fetchone()[0] or 0

        conn.close()

        # Render the dashboard template with the statistics and recent changes
        return render_template('ruhi/ruhi_dashboard.html',
...
@ruhi_bp.route('/ruhi/payment_due', methods=['GET'])
def payment_due():
    if 'ruhi' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        # Fetch customers where reference_name = 'ruhi', delivery = 'yes', and payment_status = 'pending'
        cursor.execute('''
            SELECT * FROM customers
            WHERE LOWER(reference_name) = "ruhi"
            AND LOWER(delivery) = "yes"
        ...
        return render_template('ruhi/payment_due.html', 
...
@ruhi_bp.route('/ruhi/scheme', methods=['GET', 'POST'])
def ruhi_scheme_page():
    if 'ruhi' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
    ...
    try:
        ...
        # Query to count orders grouped by amount for ruhi, filtering by date range
        cursor.execute(''' 
...
@ruhi_bp.route('/ruhi/canceled', methods=['GET'])
def ruhi_canceled_page():
    if 'ruhi' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Query to get the canceled orders for ruhi, sorted by date (most recent first)
        cursor.execute('''
        ...
        return render_template('ruhi/ruhi_canceled_page.html', canceled_orders=canceled_orders)
