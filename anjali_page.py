...
# Create a Blueprint for the anjali page
anjali_bp = Blueprint('anjali_bp', __name__)
...
@anjali_bp.route('/anjali_page')
def anjali_page():
    # Check if the user has permission to view this page
    if 'anjali' not in session['permissions'] and 'admin' not in session['permissions']:
    ...
    try:
        ...
        # Fetch records where the reference_name is 'anjali', case-insensitive
        cursor.execute('SELECT * FROM customers WHERE LOWER(reference_name) = "anjali" ORDER BY id DESC')
        ...
        return render_template('anjali/anjali.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return "An error occurred while fetching data."

@anjali_bp.route('/anjali/edit/<int:customer_id>', methods=['GET', 'POST'])
def anjali_edit_page(customer_id):
    if 'anjali' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render_template('anjali/anjali_edit.html', customer=customer_dict)
        ...
        if request.method == 'POST':
            ...
            if referrer:
                ...
                return redirect(url_for('anjali_bp.anjali_delivery_page'))  # Fallback to a default page (adjust as needed)
...
@anjali_bp.route('/anjali/delivery', methods=['GET'])
def anjali_delivery_page():
    if 'anjali' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        if search_query:
            cursor.execute(''' 
                ...
                WHERE LOWER(reference_name) = "anjali" 
        ...
        return render_template('anjali/anjali_delivery.html', customers=customers, total_amount=total_amount, total_customers=total_customers, additional_mobiles=additional_mobiles)

    except sqlite3.DatabaseError as e:
        print(f"Database error: {e}")
        return f"An error occurred while fetching data: {e}"

@anjali_bp.route('/anjali/dashboard', methods=['GET'])
def anjali_dashboard():
    if 'anjali' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
        return redirect(url_for('login'))  # Redirect to login page
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "anjali" AND LOWER(delivery) = "yes"')
        total_customers = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "anjali" AND LOWER(delivery) = "yes"')
        total_amount = cursor.fetchone()[0] or 0

        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "anjali" AND LOWER(delivery) = "no"')
        total_no_delivery = cursor.fetchone()[0]

        # Fetch total amount for "no" deliveries (including canceled orders for totals)
        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "anjali" AND LOWER(delivery) = "no"')
        total_no_delivery_amount = cursor.fetchone()[0] or 0

        # Fetch the same for canceled deliveries (to include in total)
        cursor.execute('SELECT COUNT(*) FROM customers WHERE LOWER(reference_name) = "anjali" AND payment_status = "Cancel"')
        total_cancelled = cursor.fetchone()[0]

        cursor.execute('SELECT SUM(amount) FROM customers WHERE LOWER(reference_name) = "anjali" AND payment_status = "Cancel"')
        total_cancelled_amount = cursor.fetchone()[0] or 0

        conn.close()

        # Render the dashboard template with the statistics and recent changes
        return render_template('anjali/anjali_dashboard.html',
...
@anjali_bp.route('/anjali/payment_due', methods=['GET'])
def payment_due():
    if 'anjali' not in session['permissions'] and 'admin' not in session['permissions']:
        flash('You do not have permission to access this page. Logging you out.', 'danger')
        session.pop('username', None)  # Log the user out
    ...
    try:
        ...
        # Fetch customers where reference_name = 'anjali', delivery = 'yes', and payment_status = 'pending'
        cursor.execute('''
            SELECT * FROM customers
            WHERE LOWER(reference_name) = "anjali"
            AND LOWER(delivery) = "yes"
        ...
        return render_template('anjali/payment_due.html', 
...
@anjali_bp.route('/anjali/scheme', methods=['GET', 'POST'])
def anjali_scheme_page():
    if 'anjali' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
    ...
    try:
        ...
        # Query to count orders grouped by amount for Anjali, filtering by date range
        cursor.execute(''' 
...
@anjali_bp.route('/anjali/canceled', methods=['GET'])
def anjali_canceled_page():
    if 'anjali' not in session.get('permissions', []) and 'admin' not in session.get('permissions', []):
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Query to get the canceled orders for Anjali, sorted by date (most recent first)
        cursor.execute('''
        ...
        return render_template('anjali/anjali_canceled_page.html', canceled_orders=canceled_orders)
